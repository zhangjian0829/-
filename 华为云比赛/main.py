...
model_names = ['resnext101_32x8d_wsl', 'resnext101_32x16d_wsl', 'resnext101_32x32d_wsl',
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.pretrained:
        ...
        if not mox.file.exists('../pre-trained_model/pytorch/ig_resnext101_32x16-c6f796b0.pth'):
            mox.file.copy('s3://zhangjian0829/ig_resnext101_32x16-c6f796b0.pth',
                          '../pre-trained_model/pytorch/ig_resnext101_32x16-c6f796b0.pth')
            print('copy pre-trained model from OBS to: %s success' %
                  (os.path.abspath('../pre-trained_model/pytorch/ig_resnext101_32x16-c6f796b0.pth')))
        else:
...
def save_best_checkpoint(best_acc1, args):
    ...
    mox.file.copy(os.path.join(args.deploy_script_path, 'Res.py'),
                  os.path.join(args.train_url, 'model/Res.py'))
    mox.file.copy(os.path.join(args.deploy_script_path, 'resnext_wsl.py'),
                  os.path.join(args.train_url, 'model/resnext_wsl.py'))
